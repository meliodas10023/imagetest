...
def locate_license_plate(image):
    ...
    # 定义蓝色车牌的HSV阈值
    lower_blue = np.array([90, 113, 99], dtype=np.uint8)
    upper_blue = np.array([151, 255, 255], dtype=np.uint8)
    mask = cv2.inRange(hsv, lower_blue, upper_blue)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((3, 3), np.uint8)) #  形态学处理
    # 查找轮廓
...
if __name__ == '__main__':
    ...
    print("最终识别结果:", ''.join(recognized_chars))
